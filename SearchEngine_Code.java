...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
        ...
        for (KeyValue kv : indexRow.list()) {
            ...
            byte[] byte_PageDocId = kv.getQualifier();
            String str_PageDocId = new String(byte_PageDocId);
			pageDocId = str_PageDocId;           
			byte[] byte_DocFreqValue = kv.getValue();
            int bytesFreq = Bytes.toInt(byte_DocFreqValue);
            freq = bytesFreq;
			
            // Step 2: get the URI of the page from clueWeb09DataTable
            
			Result result_Data_Row = dataTable.get(new Get(byte_PageDocId));
			byte[] byte_family_Details = Constants.CF_DETAILS_BYTES;
			byte[] byte_qualifier_Bytes=Constants.QUAL_URI_BYTES;
			byte[] byte_PageUri = result_Data_Row.getValue(byte_family_Details, byte_qualifier_Bytes);
			pageUri =  byte_PageUri.toString();
            // Step 3: get the page rank value of this page from clueWeb09PageRankTable
		//	
			Get pageRank_Obtained = new Get(byte_PageDocId);
			Result PageRankRow = prTable.get(pageRank_Obtained);
